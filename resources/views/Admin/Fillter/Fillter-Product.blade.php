...
<head>
    ...
    <title>DANH SÁCH BỘ LỌC SẢN PHẨM</title>
</head>
<body>
    ...
    <nav style="--bs-breadcrumb-divider: '/'" aria-label="breadcrumb">
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a class="nav-link" href="/Admin/Fillter/FillterList" >DANH SÁCH BÔ LỌC</a></li>
          <li class="breadcrumb-item active">
            DANH SÁCH BÔ LỌC SẢN PHẨM
          </li>
        </ol>
    </nav>
    <nav class="navbar nav-function">
        <div class="button-group group-function">
            <button class="btn btn-success show-all-fillter" >
                <a  href="{{ route('admin.product.fillter.list', ['sort' => $sort]) }}">
           ...
           <button class="btn btn-success plus-fillter" >
                <a class="nav-link" href= " {{ route('admin.product.fillter.plus')}}">
            ...
            <button class="btn btn-success show-new-fillter">
                <a class="nav-link" href="{{ route('admin.product.fillter.list', ['sort' => $sort, 'show_new' => true]) }}">FILLTER PRODUCT MỚI THÊM </a>
            </button>

            <button class="btn btn-success show-edit-fillter">
               <a class="nav-link"href="{{ route('admin.product.fillter.list', ['sort' => $sort, 'show_edited' => true]) }}">FILLTER PRODUCT VỪA CHỈNH SỬA</a>
            </button>
           <form action="{{ route('admin.product.fillter.list') }}" method="GET"class="d-flex search" role="search">
    ...
    @if($product_fillters->isEmpty())
    <div class="alert alert-info">
        @if($showNew)
            Không có bộ lọc sản phẩm nào được thêm mới trong 24 giờ qua.
        @elseif($showEdited)
            Không có bộ lọc sản phẩm nào được chỉnh sửa trong 24 giờ qua.
        @else
           Không có bộ lọc sản phẩm nào trong danh sách.
        @endif
    </div>
   @else
    ...
    <div class="pagination-margin-fillterproduct">
        <nav aria-label="Page navigation example">
            <ul class="pagination custom-pagination">
                {{ $product_fillters->appends(['product_fillters' => request('product_fillters')])->links() }}
            </ul>
        </nav>
       </div>
    @endsection
