...
<head>
    ...
    <link href="/css/Fillter_Product_Plus_Admin.css" rel="stylesheet">
    <title>   THÊM BỘ LỌC SẢN PHẨM | FILLTER PRODUCT | </title>
</head>
<body>
    @extends('Layouts.Layout_Admin')
    @section('title','Fillter-Product-Plus')
    ...
    @section('content')
      <nav class="navbar navbar-expand-lg">
        <div class="container-fluid">
          <a class="navbar-fillter-product-plus name nav-link" href=" {{ route('admin.product.fillter.list')}}" >
            ...
            THÊM BỘ LỌC SẢN PHẨM | FILLTER PRODUCT |
          </a>
      ...
      <nav style="--bs-breadcrumb-divider: '/'" aria-label="breadcrumb">
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="{{ route('admin.category.list')}}" class="nav-link">  THÊM BỘ LỌC SẢN PHẨM | FILLTER PRODUCT |</a></li>
          <li class="breadcrumb-item active"><a>  THÊM BỘ LỌC SẢN PHẨM | FILLTER PRODUCT |</a></li>
        </ol>
     </nav>
     <div class="form-FillterProductPlus">
        <form class="row g-3" action="{{ route('admin.product.fillter.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="coolinput col-md-9">
                <label for="ProductID" class="text">Sản Phẩm:</label>
                <select name="ProductID" id="ProductID" class="input form-control product" required>
            ...
            <div class="coolinput col-md-9">
                <label for="FillterID" class="text">Bộ Lọc :</label>
                <select name="FillterID[]" id="FillterID" class="input form-control fillter"  required>
                    <option value="">---Chọn Bộ Lọc---</option>
                    @foreach($fillters as $fillter)
                        <option value="{{ $fillter->id }}">{{ $fillter->name }}</option>
                    @endforeach
                </select>
            </div>
           ...
           <div class="form-group">
                ...
                <a href=" {{ route('admin.product.fillter.list') }}" class="btn btn-danger close-Category">
