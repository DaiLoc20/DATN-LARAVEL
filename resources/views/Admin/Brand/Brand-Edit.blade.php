    ...
    @section('content')
        ...
        <form action="{{ route('admin.brands.update', $brands->id) }}" method="POST">
            ...
            <div class="coolinput col-md-6">
                <label for="name" class="text">Tên Hãng Xuất:</label>
                <input type="text" class="input form-control @error('name') is-invalid @enderror" " id="name" name="name" value="{{ old('name', $brands->name) }}" required>
                @error('name')
