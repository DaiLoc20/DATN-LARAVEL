...
<body>
    ...
    @section('content')
        ...
        <form action="{{ route('admin.brands.update', $brands->id) }}" method="POST">
            ...
            <div class="mb-3">
                <label for="name" class="form-label name-Brand" >SỬA BRAND</label>
                <input type="text" class="form-control input-Brand @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $brands->name) }}" required>
                @error('name')
