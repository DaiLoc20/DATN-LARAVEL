...
<head>
    ...
    <title>Thêm Hãng Sản Xuất | BRAND |</title>
</head>
<body>
    ...
    @section('content')
        ...
        <div class="form-BrandPlus">
            <form class="row g-3" action="{{ route('admin.brands.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="coolinput col-md-12">
                    <label for="name" class="text">Tên hãng sản xuất:</label>
                    <input type="text" class="input form-control @error('name') is-invalid @enderror" id="name" name="name" value=" {{ old('name') }}" required>
                    @error('name')
