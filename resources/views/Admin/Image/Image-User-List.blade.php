...
<head>
    ...
    <title>Danh Sách Hình Ảnh Tài Khoản</title>
</head>
<body>
    ...
    @section('content')
        <nav class="navbar navbar-expand-lg">
            <div class="container-fluid">
              <a class="navbar-brand name" href="{{ route('admin.image.user.list')}}">
                ...
                <button class="btn btn-success show-all-imageBrand" >
                   ...
                   <button class="btn btn-success plus-imageBrand" >
                    ...
                    <button class="btn btn-success show-new-imageBrand">
                        <a class="nav-link" href="{{ route('admin.image.user.list', ['sort' => $sort, 'show_new' => true]) }}">HÌNH ẢNH MỚI TẠO </a>
                    </button>

                    <button class="btn btn-success show-edit-imageBrand">
                       <a class="nav-link"href="{{ route('admin.image.user.list', ['sort' => $sort, 'show_edited' => true]) }}">HÌNH ẢNH VỪA CHỈNH SỬA</a>
                    </button>
        ...
        <div class="alert alert-info">
            @if($showNew)
                Không có hình nào nào được thêm mới trong 24 giờ qua.
            @elseif($showEdited)
                Không có hình nào nào được chỉnh sửa trong 24 giờ qua.
            @else
               Không có hình nào nào trong danh sách.
            @endif
        </div>
       @else
    <table class="table">
        <thead  class="table-backgroupColor">
          <tr>
            ...
            <th scope="col" class="border-th-name">HỌ TÊN </th>
        ...
        <tbody>
          ...
          @foreach($userImages as $image)
             <tr>
                ...
                <td class="border-width-Name">{{ $image->users ? $image->users->last_name .' '.$image->users->first_name : 'N/A' }}</td>
                <td class="border-width-Img">
                    <img src="{{ asset($image->path) }}" alt="Category Image" style="max-width: 50px;">
                </td>
             </tr>
          @endforeach
        </tbody>
    ...
    <div class="pagination-margin">
        <nav aria-label="Page navigation example">
            <ul class="pagination custom-pagination">
                {{ $userImages->appends(['user_page' => request('user_page')])->links() }}
            </ul>
        </nav>
    </div>
    @endsection
