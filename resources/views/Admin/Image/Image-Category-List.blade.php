...
<head>
    ...
    <link href="/css/Image_Category_Admin.css" rel="stylesheet">
    <title>Danh Sách Hình Ảnh Loại Sản Phẩm ( Parent )</title>
</head>
<body>
    ...
    @section('content')
        <nav class="navbar navbar-expand-lg">
            <div class="container-fluid">
              <a class="navbar-brand name" href="{{ route('admin.image.brand.list')}}">
                ...
                DANH SÁCH HÌNH ẢNH LOẠI SẢN PHẨM ( PARENT )
              </a>
        ...
        <nav style="--bs-breadcrumb-divider: '/'" aria-label="breadcrumb">
            <ol class="breadcrumb">
              ...
              <li class="breadcrumb-item active"> DANH SÁCH HÌNH ẢNH LOẠI SẢN PHẨM ( PARENT )</li>
            </ol>
        </nav>
        <nav class="navbar nav-function">
            <div class="button-group group-function">
                <button class="btn btn-success show-all-imageCategory" >
                    <a  href="{{ route('admin.image.category.list', ['sort' => $sort]) }}">
                   ...
                   <button class="btn btn-success plus-imageCategory" >
                    ...
                    <button class="btn btn-success show-new-imageCategory">
                        <a class="nav-link" href="{{ route('admin.image.category.list', ['sort' => $sort, 'show_new' => true]) }}">HÌNH ẢNH MỚI THÊM</a>
                    </button>

                    <button class="btn btn-success show-edit-imageCategory">
                       <a class="nav-link"href="{{ route('admin.image.category.list', ['sort' => $sort, 'show_edited' => true]) }}">HÌNH ẢNH VỪA CHỈNH SỬA</a>
                    </button>

                   <form action="{{ route('admin.image.category.list') }}" method="GET"class="d-flex search" role="search">
        ...
        <tbody>
          ...
          @foreach($categoryImages as $category)
             <tr>
                ...
                <td class="border-width-Name">{{ $category->category ? $category->category->name : 'N/A' }}</td>
                <td class="border-width-Img">
                    <img class="img-brand" src="{{ $category->path }}" alt="{{ $category->name }}">
                </td>
             </tr>
          @endforeach
        </tbody>
