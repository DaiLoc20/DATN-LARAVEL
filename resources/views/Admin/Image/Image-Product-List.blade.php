...
<head>
    ...
    <link href="/css/Image_Product_Admin.css" rel="stylesheet">
...
<body>
    ...
    @section('content')
        <nav class="navbar navbar-expand-lg">
            <div class="container-fluid">
              <a class="navbar-brand name" href="{{ route('admin.image.product.list')}}">
                ...
                DANH SÁCH HÌNH ẢNH SẢN PHẨM | PRODUCT |
              </a>
        ...
        <nav style="--bs-breadcrumb-divider: '/'" aria-label="breadcrumb">
            <ol class="breadcrumb">
              ...
              <li class="breadcrumb-item active">DANH SÁCH HÌNH ẢNH SẢN PHẨM | PRODUCT | </li>
            </ol>
        </nav>
            ...
            <button class="btn btn-success show-all-imageBrand" >
               ...
               <button class="btn btn-success plus-imageBrand" >
                ...
                <button class="btn btn-success show-new-imageBrand">
                    <a class="nav-link" href="{{ route('admin.image.product.list', ['sort' => $sort, 'show_new' => true]) }}">PRODUCT MỚI THÊM</a>
                </button>

                <button class="btn btn-success show-edit-imageBrand">
                   <a class="nav-link"href="{{ route('admin.image.product.list', ['sort' => $sort, 'show_edited' => true]) }}">PRODUCT VỪA CHỈNH SỬA</a>
                </button>

               <form action="{{ route('admin.image.product.list') }}" method="GET"class="d-flex search" role="search">
    ...
    <div class="alert alert-info">
        @if($showNew)
            Không có hình ảnh nào được thêm mới trong 24 giờ qua.
        @elseif($showEdited)
            Không có hình ảnh nào được chỉnh sửa trong 24 giờ qua.
        @else
           Không có hình ảnh nào trong danh sách.
        @endif
    </div>
   @else
    <table class="table">
        <thead  class="table-backgroupColor">
          <tr>
            ...
            <th scope="col" class="border-th-stt">IMAGE ID</th>
            <th scope="col" class="border-th-stt">PRODUCT ID</th>
            <th scope="col" class="border-th-name">TÊN | PRODUCT |</th>
            <th scope="col" class="border-th-img">HÌNH ẢNH</th>
          </tr>
        </thead>
        <tbody>
          @php
            $stt = ($productImages->currentPage() - 1) * $productImages->perPage() + 1;
          @endphp
          @foreach($productImages as $product)
             <tr>
                <td class="border-width-STT">{{$stt++}}</td>
                <td class="border-width-STT">{{ $product->id }}</td>
                <td class="border-width-STT">{{ $product->ProductID }}</td>
                <td class="border-width-Name">{{ $product->products ? $product->products->name : 'N/A' }}</td>
                <td class="border-width-Img">
                    <img class="img-product" src="{{  $product->path  }}" alt="{{ $product->name }}">
                </td>
             </tr>
          @endforeach
        </tbody>
    ...
    <div class="pagination-margin">
        <nav aria-label="Page navigation example">
            <ul class="pagination custom-pagination">
                {{ $productImages->appends(['product_page' => request('productImages')])->links() }}
            </ul>
            </ul>
        </nav>
    </div>
